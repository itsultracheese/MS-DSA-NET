...
5. Run seg_fcd_test.py using python to predict the FCD lesions. During runing, raw images are first preprocessed using FSL and the result images are saved in "./inputs/raw/fsl/sub-xxxxx“ (t1_reg.nii.gz, flair_reg.nii.gz along with other temporary files ). The result images (t1_reg.nii.gz, flair_reg.nii.gz) are loaded and fed into the proposed model for prediction. The predicted lesion map would saved in the "outputs/date/sub-xxxxx" with name "t1_reg_seg.nii.gz". 
